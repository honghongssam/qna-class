...
class LocalStorageAdapter {
    ...
    // [로컬 모드] 프로필 아바타 이미지 변경
    async updateUserPhotoURL(photoURL) {
        if (!isClient) return null;
        ...
        if (savedUser) {
            ...
            // 가상 회원 DB 내의 정보도 영구 업데이트하여, 재로그인 시에도 아바타 유지
            const users = this._getLocalUsers();
            const userIndex = users.findIndex(u => u.email === savedUser.email);
            if (userIndex !== -1) {
            ...
            // 로그인 상태 리스너 브로드캐스트
            this.authListeners.forEach(cb => cb(savedUser));
            return savedUser;
        }
...
class FirebaseAdapter {
    ...
    // [실제 모드] 실제 프로필 아바타 이미지 변경
    async updateUserPhotoURL(photoURL) {
        if (!this.auth.currentUser) {
            throw new Error("로그인된 사용자를 찾을 수 없습니다.");
        }
        // Firebase Auth 상의 currentUser 프로필 이미지 정보 영구 갱신
        await updateProfile(this.auth.currentUser, {
            photoURL: photoURL
        });
        
        // 갱신된 회원 정보 구성 후 리스너 전파
        const updatedUser = {
